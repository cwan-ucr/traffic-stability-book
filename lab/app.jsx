...
function App() {
  ...
  const applicationMeta = applicationCase === "open" ? {
    ...
    setup: "12 km 单车道高速公路，法定最高限速 120 km/h；取短车头时距 T=0.7 s、净间距 18 m，事件前均匀速度约 75 km/h。0–1200 s 持续到达（1087 辆新车进入）；慢车在 x≈6 km 处于 300–420 s 降至约 65 km/h，VSL 固定在 x=5–6 km。",
    conclusion: "120 km/h 上限下，短时距工作点 Φ=−0.001415 s⁻²，处于长波失稳侧。在线只在 114–120 km/h 内最大化 Φ，选择 114 km/h 后 Φ=+0.002396 s⁻²，工作点跨入稳定侧；随后再用连续流仿真事后评价排队、安全、延误和能耗。",
  };
  const applicationBenefits = !applicationPair ? [] : applicationCase === "open" ? [
    ...
    ["形成阶段排队尾波速度", "−2.69 → −0.35 m/s", "逆向传播减缓 86.9%"],
    ["低速车辆峰值", "121 → 100 辆", "下降 17.4%"],
    ["峰值速度离差", "8.912 → 3.152 m/s", "下降 64.6%"],
    ["最小 TTC", "2.34 → 11.80 s", "提高 404%"],
    ["累计延误", "6.866 → 4.588 veh·h", "下降 33.2%"],
    ["Wu 模型单位里程电耗", "17.521 → 17.304 kWh/100km", "下降 1.24%"],
  ];
                ...
                <Card title="管理含义" note={applicationCase === "open" ? "稳定性收益" : "控制收益与代价"}>
                  <div className="tsl-live-note" style={{ marginTop: 0 }}>
                    {applicationCase === "open"
                      ...
                      : "法定上限仍是 120 km/h；真正导致失稳的是短时距 T=0.7 s 与高密度工作点。慢车的小幅降速触发后，无控制排队尾部以 −2.69 m/s 向上游传播；114 km/h VSL 把 Φ 从 −0.001415 推到 +0.002396，使尾波速度降到 −0.35 m/s、速度离差下降 64.6%、延误下降 33.2%。除 Φ 外均为事后读数。"}
                  </div>
                </Card>
              </div>
            </div>
